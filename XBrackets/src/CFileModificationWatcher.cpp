/*
This file is originally a part of NppExec (in 2013).
Adapted to XBrackets Lite (in 2025).
Copyright (C) 2013, 2025, DV <dvv81 (at) ukr (dot) net>
...
namespace
{
    tstr getFileDir(LPCTSTR cszFilePath, LPCTSTR* ppFileName = nullptr)
    {
        ...
        size_t nPos = sDir.find_last_of(_T("\\/"));
        if ( nPos != tstr::npos )
        {
            sDir.erase(nPos, sDir.length() - nPos);
            if ( ppFileName )
